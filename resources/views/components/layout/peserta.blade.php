...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'Ruang Dampak') }} - Dashboard Peserta</title>
    ...
    <body class="font-body text-neutral-900 antialiased bg-neutral-50 flex min-h-screen">
        ...
        <!-- Sidebar -->
        <x-layout.sidebar-peserta />

        <!-- Main Content Wrapper -->
        <div class="flex-1 flex flex-col min-h-screen overflow-hidden">
            <!-- Mobile Header (Visible only on md and smaller) -->
            <header class="lg:hidden h-16 bg-primary-950 border-b border-primary-900 flex items-center justify-between px-4">
                ...
                <button type="button" class="text-white hover:text-accent-teal focus:outline-none" onclick="document.dispatchEvent(new CustomEvent('toggle-sidebar'))">
